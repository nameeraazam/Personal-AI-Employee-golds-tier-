...
class Config:
    ...
    # Monitoring settings
    CHECK_INTERVAL = int(os.getenv('LINKEDIN_CHECK_INTERVAL', '60'))  # seconds
    ...
    # Sales lead keywords (case insensitive)
    SALES_KEYWORDS = [
...
"""
    ...
    def _is_logged_in(self) -> bool:
        ...
        try:
            # Check for LinkedIn-specific elements that indicate login
            self.page.wait_for_selector('div.global-nav', timeout=5000)
            
            # Also check URL doesn't contain login
    ...
    def _check_messages_sync(self) -> List[Dict]:
        """Check for new messages (sync wrapper)"""
    ...
    def _check_notifications(self) -> List[Dict]:
        ...
        try:
            ...
            for notif in notifications[:15]:  # Check last 15 notifications
                try:
    ...
    def process_new_items(self, messages: List[Dict], notifications: List[Dict], 
        ...
        """Process new items and create action files"""
        ...
        try:
            ...
            if session_loaded:
                ...
                if not self._is_logged_in():
                    ...
                    self.page.run_async(lambda p: self._login())
            else:
            ...
            while True:
                ...
                if self.config.MONITOR_MESSAGES:
                    messages = self._check_messages_sync()
            ...
            self.logger.info(f"Sales leads identified: {self.stats['sales_leads']}")
